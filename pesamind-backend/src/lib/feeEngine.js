 ...
 */
async function previewCoverage(transactionTypeCode, userId) {
  ...
  if (exemption) return { covered: true, reason: `Fee waived: ${exemption.reason}.` };
  ...
  for (const sub of subs) {
    const included = Array.isArray(sub.bundle.includedTransactionTypeIds) ? sub.bundle.includedTransactionTypeIds : [];
    if (included.includes(type.id)) return { covered: true, reason: `Covered by your ${sub.bundle.nameEn} bundle.` };
  }
