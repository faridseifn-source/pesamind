...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [snapshot, wallet] = await Promise.all([
      getCardIssuingProvider().debit(card.id, { type: "lipa", amount, label: `Paid to ${recipient.name}` }),
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const card = await myCard(req.userId);
    const snapshot = await getCardIssuingProvider().debit(card.id, { type: "gepg", amount, label: biller, sub: `Control No. ${control}` });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const card = await myCard(req.userId);
    const snapshot = await getCardIssuingProvider().debit(card.id, { type: "luku", amount, label: `Meter ${meter}` });
