...
// successful top-up.
async function recordTopupTransaction(userId, amount, maskedSource, source) {
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      await recordTopupTransaction(req.userId, amount, funding.maskedSource, "topup_gateway");
    } catch (err) {
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      await recordTopupTransaction(req.userId, amount, funding.maskedSource, "topup_oct");
    } catch (err) {
      console.error("Failed to record top-up as a Transaction (card was still credited successfully)", err); // eslint-disable-line no-console
    }
    const body = { card: snapshot, funding };
