...
// POST /wallets/:id/invite { phone } — owner only, invites a registered user by phone.
router.post(
  "/:id/invite",
  asyncHandler(async (req, res) => {
    const { phone } = z.object({ phone: z.string().min(8).max(9) }).parse(req.body);
    const wallet = await prisma.wallet.findUnique({ where: { id: req.params.id }, include: { members: true } });
    if (!wallet) throw notFound("Wallet not found");
    ...
    const invite = await prisma.walletInvite.create({
      data: { walletId: wallet.id, invitedUserId: invitee.id, invitedByUserId: req.userId },
    });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await writeAudit(req.userId, "wallet.invite_accepted", { ip: req.ip, walletId: invite.walletId });
    await prisma.notification.create({
