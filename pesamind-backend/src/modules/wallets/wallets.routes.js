...
const { genCardCredentials } = require("../virtual-cards/cardCredentials");
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await writeAudit(req.userId, "wallet.invite_accepted", { ip: req.ip, walletId: invite.walletId, addOnCardCreated: !!invite.createAddOnCard });
    await prisma.notification.create({
      data: { userId: invite.invitedByUserId, type: "wallet_invite_accepted", title: "Invitation accepted", message: "Your wallet invitation was accepted." },
    });
    if (invite.createAddOnCard) {
      await prisma.notification.create({
        data: { userId: req.userId, type: "virtual_card_issued", title: "New add-on card", message: "A shared add-on card is now available to you." },
      });
    }
