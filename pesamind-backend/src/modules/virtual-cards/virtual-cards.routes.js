...
const { genCardCredentials } = require("./cardCredentials");
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const card = await prisma.virtualCard.create({
      data: { walletId, ownerId: req.userId, holderId: holderUserId, type: "parent_linked", label: label || null, last4, fullNumberEnc: encryptField(fullNumber), cvvEnc: encryptField(cvv), expiry: "09/29" },
    });
    await writeAudit(req.userId, "virtualcard.created.parent_linked", { ip: req.ip, walletId, holderUserId, cardId: card.id });
    await prisma.notification.create({
      data: { userId: holderUserId, type: "virtual_card_issued", title: "New add-on card", message: "A shared add-on card has been issued to you." },
    });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { last4, fullNumber, cvv } = genCardCredentials();
    const card = await prisma.virtualCard.create({
      data: { walletId: personalMembership.walletId, ownerId: req.userId, holderId: req.userId, type: "independent", label: label || null, last4, fullNumberEnc: encryptField(fullNumber), cvvEnc: encryptField(cvv), expiry: "09/29" },
    });
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!card.fullNumberEnc || !card.cvvEnc) {
      const { fullNumber, cvv } = genCardCredentials();
