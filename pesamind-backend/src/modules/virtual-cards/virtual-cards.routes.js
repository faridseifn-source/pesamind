...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { amount, merchant, categoryId } = z
      .object({ amount: z.number().positive(), merchant: z.string().min(1), categoryId: z.string().min(1) })
      .parse(req.body);
    ...
    const [updated, , transaction] = await prisma.$transaction([
      ...
      prisma.virtualCardActivity.create({ data: { cardId: card.id, type: "spend", amount, label: `Paid ${merchant}`, performedByUserId: req.userId } }),
    ...
    await saveIdempotentResponse(req, 200, body);
    await writeAudit(req.userId, "virtualcard.spend", { ip: req.ip, cardId: card.id, amount, merchant });
    res.json(body);
  })
);
