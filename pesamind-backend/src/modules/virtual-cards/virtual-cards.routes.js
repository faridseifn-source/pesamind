...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (issuanceFee > 0) {
      ...
      await collectFee(feeQuote.id, `CARDFEE-${card.id}`).catch(() => {});
    }
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.query.format === "csv") {
      ...
      if (fee > 0) {
        ...
        await collectFee(quote.id, `STMTFEE-${card.id}-${Date.now()}`).catch(() => {});
      }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await collectFee(quote.id, reference).catch(() => {});
    const body = { card: serializeCard(updated, req.userId), transaction, reference, fee: { pesaMindFee: Number(quote.pesaMindFee), tax: Number(quote.tax) } };
    await saveIdempotentResponse(req, 200, body);
    await writeAudit(req.userId, "virtualcard.pay.gepg", { ip: req.ip, cardId: card.id, amount, control, reference, pesaMindFee: Number(quote.pesaMindFee) });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await collectFee(quote.id, reference).catch(() => {});
    const body = { card: serializeCard(updated, req.userId), transaction, reference, token, fee: { pesaMindFee: Number(quote.pesaMindFee), tax: Number(quote.tax) } };
    await saveIdempotentResponse(req, 200, body);
    await writeAudit(req.userId, "virtualcard.pay.luku", { ip: req.ip, cardId: card.id, amount, meter, reference, pesaMindFee: Number(quote.pesaMindFee) });
