...
// GET /virtual-cards/:id/statement?from=&to= — "pull statement" for an add-on card.
router.get(
  "/:id/statement",
  asyncHandler(async (req, res) => {
    const card = await loadCard(req.params.id);
    ...
    await writeAudit(req.userId, "virtualcard.statement_pulled", { ip: req.ip, cardId: card.id, from, to });
    res.json(statement);
  })
);
