...
const { unauthorized, badRequest, locked } = require("../../lib/errors");
...
// A fixed, sensible list rather than all 160+ ISO currencies — this is a
// TZS-native app for a Tanzanian audience; these are the currencies a
// customer here would plausibly actually encounter (home currency plus
// common travel/remittance destinations), not a generic global picker.
const SUPPORTED_CURRENCIES = ["TZS", "USD", "KES", "UGX", "ZAR", "EUR", "GBP", "RWF"];
...
router.patch(
  ...
  requireAuth,
  asyncHandler(async (req, res) => {
    const { currency } = z.object({ currency: z.enum(SUPPORTED_CURRENCIES) }).parse(req.body);
