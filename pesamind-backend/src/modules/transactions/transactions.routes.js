...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (currency && currency !== requester.preferredCurrency) {
      try {
        amountFields = await convertToPreferredCurrency({ amount: input.amount, currency, preferredCurrency: requester.preferredCurrency });
      } catch (err) {
        console.error("Currency conversion failed for manual transaction entry:", err.message); // eslint-disable-line no-console
        throw badRequest(`Couldn't convert from ${currency} right now — please enter the amount in ${requester.preferredCurrency} instead, or try again shortly.`);
      }
    }
    delete rest.amount;
