...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const feePreview = isOnUs ? { feeAmount: 0, feeType: "none", institutionKnown: true, transfersEnabled: true } : await previewInstitutionFee(merchant.acquirerId, parsed.amount);
    res.json({
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const feePreview = isOnUs ? { feeAmount: 0, feeType: "none", institutionKnown: true, transfersEnabled: true } : await previewInstitutionFee(merchant.acquirerId, null);
    res.json({
      merchant: { id: merchant.id, name: merchant.name, city: merchant.city, mcc: merchant.mcc },
      isOnUs,
