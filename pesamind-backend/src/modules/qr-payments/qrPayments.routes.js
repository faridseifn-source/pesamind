...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (parsed.amountFixed && transfersActuallyEnabled) {
      ...
      fee = {
        pesaMindFee: Number(quote.pesaMindFee), partnerFee: Number(quote.partnerFee), tax: Number(quote.tax), totalFee: Number(quote.totalFee),
      ...
      if (coverage.covered) {
        fee = { pesaMindFeeRule: coverage.wouldBeRule, coveredMessage: coverage.reason, coveredMessageSw: coverage.reasonSw, partnerFee: partnerFeePreview, transfersEnabled: transfersActuallyEnabled };
      } else {
        const rulePreview = await previewFeeRule(transactionTypeCode, previewCtx);
        fee = { pesaMindFeeRule: rulePreview, partnerFee: partnerFeePreview, transfersEnabled: transfersActuallyEnabled, suggestedBundle: serializeSuggestedBundle(coverage.suggestedBundle) };
      }
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const fee = coverage.covered
      ? { pesaMindFeeRule: coverage.wouldBeRule, coveredMessage: coverage.reason, coveredMessageSw: coverage.reasonSw, partnerFee: partnerFeePreview, transfersEnabled: transfersActuallyEnabled }
      : { pesaMindFeeRule: await previewFeeRule(transactionTypeCode, previewCtx), partnerFee: partnerFeePreview, transfersEnabled: transfersActuallyEnabled, suggestedBundle: serializeSuggestedBundle(coverage.suggestedBundle) };
