...
async function completePaymentFromRecord(payment, merchantName) {
  ...
  const totalDebit = Number(payment.amount) + Number(payment.feeAmount || 0);
  ...
  await writeAudit(payment.userId, "qrpayment.completed", { paymentId: payment.id, reference: payment.reference, amount: Number(payment.amount), feeAmount: Number(payment.feeAmount || 0) });
  const feeNote = Number(payment.feeAmount) > 0 ? ` (includes ${Number(payment.feeAmount).toLocaleString()} TZS transfer fee)` : "";
...
async function payViaQr({ userId, rawPayload, aliasMerchantId, cardType, cardId, amountOverride, categoryId, authMethod }) {
  ...
  // its fee, which the customer pays on top of the amount. Checked BEFORE
  // the balance check below, since the true amount leaving the wallet is
  // amount + fee, not just amount.
  let feeAmount = 0;
  if (!isOnUs) {
    const institution = await assertTransferAllowed({ acquirerId: merchant.acquirerId, userId, amount });
    feeAmount = calculateTransferFee(institution, amount);
  }
  ...
  const payment = await prisma.qrPayment.create({
    data: {
      ...
      rawPayload: storedPayload, currency: "TZS", amount, feeAmount, isOnUs, categoryId,
  ...
  await logEvent(payment.id, "initiated", "success", { reference, isOnUs, feeAmount, acquirerId: merchant.acquirerId, merchantId: merchant.merchantId, viaAlias: !rawPayload });
  ...
  if (feeAmount > 0) {
    await cbs.debitSettlementAccount({ reference: `${reference}-FEE`, amount: feeAmount, narrative: `Transfer fee — ${merchant.name}`, paymentId: payment.id })
      .then((feeResult) => logEvent(payment.id, "fee_recorded", feeResult.status === "posted" ? "success" : "failed", { ...feeResult, feeAmount }))
