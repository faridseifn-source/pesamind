...
const { parseTanqrPayload } = require("../../lib/tanqr");
...
async function resolveQrPayload(rawPayload) {
  ...
  const merchant = await prisma.merchant.upsert({
    ...
    update: { name: parsed.merchantName, city: parsed.merchantCity, mcc: parsed.mcc, isOnUs },
    create: { acquirerId: parsed.acquirerId, merchantId: parsed.merchantId, name: parsed.merchantName, city: parsed.merchantCity, mcc: parsed.mcc, isOnUs },
  });
 ...
 */
async function payViaQr({ userId, rawPayload, cardType, cardId, amountOverride, categoryId, authMethod }) {
  const { parsed, merchant, isOnUs } = await resolveQrPayload(rawPayload);
  ...
  const payment = await prisma.qrPayment.create({
    data: {
      userId, reference, merchantId: merchant.id, pointOfInitiationMethod: parsed.pointOfInitiationMethod,
      rawPayload, currency: "TZS", amount, isOnUs, categoryId,
  ...
  await logEvent(payment.id, "initiated", "success", { reference, isOnUs, acquirerId: merchant.acquirerId, merchantId: merchant.merchantId });
...
module.exports = { resolveQrPayload, payViaQr, getPaymentStatus, generateReference };
