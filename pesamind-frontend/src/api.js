...
const wallets = {
  ...
  async invite(walletId, phone) {
    const { invite } = await raw(`/wallets/${walletId}/invite`, { method: "POST", body: { phone } });
    return invite;
  },
  async myInvites() {
  ...
  async sentInvites(walletId) {
    const { invites } = await raw(`/wallets/${walletId}/invites`);
    return invites;
  },
...
const virtualCards = {
  ...
  spend: (id, amount, merchant, categoryId) => raw(`/virtual-cards/${id}/spend`, { method: "POST", body: { amount, merchant, categoryId }, extraHeaders: { "Idempotency-Key": newIdempotencyKey() } }),
};
...
export const api = { auth, categories, wallets, transactions, budgets, kyc, cards, virtualCards, notifications, clearAccessToken };
