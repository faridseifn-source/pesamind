...
const auth = {
  ...
  async setCurrency(currency) {
    const { user } = await raw("/auth/me/currency", { method: "PATCH", body: { currency } });
    return user;
  },
...
const transactions = {
  ...
  downloadExportCsv: (from, to) => downloadFile(`/transactions?${new URLSearchParams({ ...(from ? { from } : {}), ...(to ? { to } : {}), format: "csv" })}`, `pesamind-transactions-${from}-to-${to}.csv`),
  async create(tx, ctx) {
