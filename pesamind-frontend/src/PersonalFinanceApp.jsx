...
function InsightsPage({ t, transactions, budgets, categories, tr, goBack, onViewCategoryPerformance }) {
  ...
  return (
    <div className="px-4 pt-2 pb-6 space-y-3">
      ...
      {/* Compact category summary — replaces the old multi-panel chart to cut scrolling; full breakdown lives on its own page now */}
      <Card t={t} className="p-4">
        <SectionLabel t={t}>Spending by category</SectionLabel>
        <div className="flex gap-2 mt-2 mb-3">
          <div className="flex-1 rounded-[14px] p-3" style={{ background: t.cardSoft }}>
            <p className="text-[10.5px]" style={{ color: t.inkFaint }}>{tr("monthlyIncome")}</p>
            <p className="text-[15px] font-bold mt-0.5" style={{ color: t.good, fontFamily: "'IBM Plex Mono', monospace" }}>{fmt(income)}</p>
          </div>
          <div className="flex-1 rounded-[14px] p-3" style={{ background: t.cardSoft }}>
            <p className="text-[10.5px]" style={{ color: t.inkFaint }}>{tr("monthlyExpenses")}</p>
            <p className="text-[15px] font-bold mt-0.5" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmt(Math.abs(expenses))}</p>
          </div>
        </div>
        <button onClick={onViewCategoryPerformance} className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-[12px] text-[13px] font-semibold" style={{ background: t.accentSoft, color: t.accent }}>
          View category performance <ChevronRight size={14} />
        </button>
      </Card>
