...
// pivot to fmtTZS(). Rather than risk 38 individual call-site edits, this
// makes fmt an alias for fmtTZS directly: one contained fix instead of
// many risky ones, and the two formatters can never drift apart again.
...
function CurrencyPreferenceCard({ t }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(err.message || "Couldn't load currency settings.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const change = async (currency) => {
    ...
    try {
      ...
      setCurrent(user.preferredCurrency);
      setSaved(true);
      setTimeout(() => setSaved(false), 2500);
    } catch (err) {
      setError(err.message || "Couldn't update your currency.");
    } finally {
      setSaving(false);
    }
  };
