...
function InviteSheet({ t, open, onClose, walletId, onInvited }) {
  ...
  const send = async () => {
    ...
    setSending(true); setError("");
    try {
      await api.wallets.invite(walletId, phone);
      setSent(true);
      onInvited?.();
    } catch (err) {
  ...
  useEffect(() => { if (open) { setPhone(""); setError(""); setSent(false); } }, [open]);
    ...
    <BottomSheet t={t} open={open} onClose={onClose} title="Invite a family member">
      ...
      {sent ? (
        <div className="flex flex-col items-center py-6 text-center">
          ...
          <p className="text-[12.5px] mt-1" style={{ color: t.inkFaint }}>They'll see it next time they check their wallet invites.</p>
        </div>
      ) : (
        <>
  ...
  return (
    <div className="px-4 pt-2 pb-24 relative">
      ...
      <button onClick={() => setActive("virtualcards")} className="w-full flex items-center justify-between p-4 rounded-[18px] mb-4" style={{ background: t.cardSoft, border: `1px solid ${t.border}` }}>
        <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: t.accentSoft }}><CreditCard size={16} color={t.good} /></div><div className="text-left"><p className="text-[13.5px] font-semibold" style={{ color: t.ink }}>Virtual cards</p><p className="text-[11.5px]" style={{ color: t.inkFaint }}>Issue and manage linked or personal virtual cards</p></div></div>
...
function VirtualCardDetailSheet({ t, card, onClose, categories, myUserId, holderName, onChanged }) {
  ...
  useEffect(() => {
    ...
    setTopupAmt(""); setSpendAmt(""); setSpendMerchant(""); setSpendCategory(""); setError("");
  ...
  return (
    <BottomSheet t={t} open={!!card} onClose={onClose} title={card.label || (card.type === "parent_linked" ? (canManage ? `${holderName || "Member"}'s card` : "Your linked card") : "Independent card")}>
      ...
      {canSpend && !card.terminated && (
        <Card t={t} className="p-4 mb-4">
          <p className="text-[12px] font-semibold mb-2.5" style={{ color: t.inkFaint }}>Make a purchase</p>
          <Field t={t} label="Merchant"><input style={inputStyle(t)} value={spendMerchant} onChange={(e) => setSpendMerchant(e.target.value)} placeholder="e.g. Java House" /></Field>
          ...
          <PrimaryButton t={t} disabled={!spendAmt || !spendMerchant || !spendCategory || busy || card.frozen} onClick={() => run(() => api.virtualCards.spend(card.id, parseFloat(spendAmt), spendMerchant, spendCategory))}>
            {card.frozen ? "Card is frozen" : busy ? "Processing…" : "Pay"}
          </PrimaryButton>
        </Card>
      )}
      ...
      {canManage && !card.terminated && (
        <>
          <Card t={t} className="p-4 mb-3">
            <p className="text-[12px] font-semibold mb-2.5" style={{ color: t.inkFaint }}>Top up from your main card</p>
            <div className="flex gap-2">
              <input style={{ ...inputStyle(t), flex: 1 }} type="number" value={topupAmt} onChange={(e) => setTopupAmt(e.target.value)} placeholder="0" />
              <button disabled={!topupAmt || busy} onClick={() => run(() => api.virtualCards.topup(card.id, parseFloat(topupAmt)))} className="px-4 rounded-[13px] font-semibold text-[13px]" style={{ background: t.accent, color: "#fff" }}>Top up</button>
            </div>
          </Card>
        ...
        <Card t={t} className="px-3">
          {activity.length ? activity.map((a) => (
            <div key={a.id} className="flex items-center justify-between py-3" style={{ borderBottom: `1px solid ${t.border}` }}>
              ...
              {a.amount !== null && <p className="text-[13px] font-semibold" style={{ color: a.type === "topup" ? t.good : t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{a.type === "topup" ? "+" : "-"}{fmtTZS(a.amount)}</p>}
            </div>
