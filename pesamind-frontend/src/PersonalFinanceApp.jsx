...
function StatementCard({ t, statement, loading, monthLabel, onPrevMonth, onNextMonth }) {
  return (
    <Card t={t} className="p-4">
      ...
      {loading ? (
        ...
        <>
          ...
          <p className="text-[10.5px] mb-2" style={{ color: t.inkFaint }}>Ref {statement.reference} · {statement.entryCount} transaction{statement.entryCount === 1 ? "" : "s"}</p>
...
function BiometricConfirmStep({ t, amount, merchantName, onConfirmed, onCancel }) {
  ...
  if (checking) return <div className="flex justify-center py-10"><RefreshCw size={18} className="animate-spin" color={t.accent} /></div>;

  return (
    <div className="text-center py-4">
      <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: t.accentSoft }}>
        <Fingerprint size={28} color={t.good} />
      </div>
      <p className="text-[14px] font-semibold mb-1" style={{ color: t.ink }}>Confirm payment</p>
      <p className="text-[12.5px] mb-6" style={{ color: t.inkFaint }}>{fmtTZS(amount)} to {merchantName}</p>
      {error && <p className="text-[12px] mb-3" style={{ color: t.danger }}>{error}</p>}
      {available ? (
        <PrimaryButton t={t} onClick={confirmBiometric} disabled={busy}>{busy ? "Waiting…" : "Confirm with Face ID / fingerprint"}</PrimaryButton>
      ) : (
        <PrimaryButton t={t} onClick={() => onConfirmed("pin")}>Confirm</PrimaryButton>
      )}
...
function QrPaymentResult({ t, payment, merchant, onDone, onRetry }) {
  ...
  return (
    <div className="flex flex-col items-center py-6 text-center">
      <div className="w-16 h-16 rounded-full flex items-center justify-center mb-4" style={{ background: success ? t.accentSoft : pending ? t.goldSoft : t.dangerSoft }}>
        {success ? <Check size={28} color={t.good} /> : pending ? <RefreshCw size={26} className="animate-spin" color={t.gold} /> : <X size={28} color={t.danger} />}
      </div>
      <p className="text-[16px] font-bold mb-1" style={{ color: t.ink }}>
        {success ? "Payment successful" : pending ? "Payment pending" : payment.status === "reversed" ? "Payment reversed" : "Payment failed"}
      </p>
      ...
      <Card t={t} className="w-full p-4 mb-4 text-left">
        <div className="flex justify-between py-1.5"><span className="text-[11.5px]" style={{ color: t.inkFaint }}>Reference</span><span className="text-[11.5px] font-semibold font-mono" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{payment.reference}</span></div>
      ...
      {failed ? (
        <PrimaryButton t={t} onClick={onRetry}>Try again</PrimaryButton>
      ) : (
        <PrimaryButton t={t} onClick={onDone}>Done</PrimaryButton>
      )}
    </div>
  );
}

// The full BOT TANQR / EMVCo customer journey: camera scan → payload
// validation → merchant confirmation → amount → risk-based auth →
// settlement → receipt. See docs/TANQR_PAYMENT_FLOW.md in the backend for
// the design this implements against.
function QrPaymentSheet({ t, open, onClose, cardType, cardId, categories, kycVerified, setKycVerified, onSuccess }) {
  const [phase, setPhase] = useState("scan"); // scan | confirm | otp | biometric | processing | result
  ...
  useEffect(() => {
    if (open) {
      setPhase("scan"); setResolved(null); setPayload(null); setAmount(""); setError(""); setScanError(""); setManualPayload(""); setResult(null);
      setCategoryId(categories.find((c) => c.id)?.id || "");
      api.qr.devSamples().then(setSamples).catch(() => {});
    }
  }, [open]);
  ...
  useEffect(() => {
    if (!open || phase !== "scan") { stopCamera(); return; }
    let cancelled = false;
    (async () => {
      try {
        ...
        setScanError("Camera access denied or unavailable. Paste a code or try a sample below.");
      }
    })();
  ...
  const goToAuth = () => {
    ...
    setError("");
    setPhase(amt > KYC_THRESHOLD ? "otp" : "biometric");
  };

  const finalizePay = async (authMethod) => {
    setPhase("processing"); setError("");
    try {
      const body = { payload, cardType, cardId, categoryId, authMethod };
  ...
  return (
    <BottomSheet t={t} open={open} onClose={onClose} title="Scan to pay">
      {phase === "scan" && (
        <div>
          {!scanError ? (
      ...
      {phase === "biometric" && <BiometricConfirmStep t={t} amount={amt} merchantName={resolved?.merchant?.name} onConfirmed={(method) => finalizePay(method)} onCancel={() => setPhase("confirm")} />}
...
function PayPage({ t, transactions, categories, categoryIdByName, refreshCardAndTransactions, walletsList, user, cardBalance, setCardBalance, cardLast4, cardActivity, addActivity, cardFrozen, setCardFrozen, cardControls, setCardControls, dailyLimit, setDailyLimit, kycVerified, setKycVerified, tr }) {
  ...
  const actions = [
    { key: "scanqr", icon: ScanLine, label: "Scan QR", color: t.accent },
  ...
  const vcActions = [
    { key: "scanqr", icon: ScanLine, label: "Scan QR" },
  ...
  return (
    <div className="px-4 pt-2 pb-8">
        ...
        <div className="pt-2">
          {isMain && (
            <>
              ...
              {subTab === "statement" && <StatementCard t={t} statement={statement} loading={statementLoading} monthLabel={monthLabel} onPrevMonth={() => shiftMonth(-1)} onNextMonth={() => shiftMonth(1)} />}
      ...
      <QrPaymentSheet
        t={t} open={sheet === "scanqr"} onClose={() => setSheet(null)}
...
function AdminSettingsTab({ t }) {
  ...
  const LABELS = { household_max_members: "Max household members", card_bin: "Card BIN prefix", cms_provider_label: "CMS provider label" };
  const HELP = {
