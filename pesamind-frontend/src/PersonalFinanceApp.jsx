...
function PayCardTile({ t, selected, onSelect, label, sublabel, badge, balance, gradientFrom, gradientTo, frozen, terminated }) {
  return (
    <button onClick={onSelect} className="shrink-0 w-[210px] snap-center text-left p-4 rounded-[20px] transition-transform" style={{ background: terminated ? t.bgSoft : `linear-gradient(135deg, ${gradientFrom}, ${gradientTo})`, border: selected ? `2px solid ${t.ink}` : "2px solid transparent", opacity: terminated ? 0.6 : 1, transform: selected ? "scale(1)" : "scale(0.97)" }}>
      <div className="flex items-center justify-between mb-3">
        <CreditCard size={18} color={terminated ? t.inkFaint : "#fff"} />
        <div className="flex items-center gap-1">
          {frozen && !terminated && <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: "rgba(255,255,255,0.25)", color: "#fff" }}>FROZEN</span>}
          {badge && <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: terminated ? t.dangerSoft : "rgba(255,255,255,0.25)", color: terminated ? t.danger : "#fff" }}>{badge}</span>}
        </div>
      </div>
      <p className="text-[12px] font-semibold truncate" style={{ color: terminated ? t.inkFaint : "#fff" }}>{label}</p>
      <p className="text-[10.5px] mb-2 truncate" style={{ color: terminated ? t.inkFaint : "rgba(255,255,255,0.7)" }}>{sublabel}</p>
      <p className="text-[17px] font-bold" style={{ color: terminated ? t.ink : "#fff", fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(balance)}</p>
    </button>
  );
}

function PayPage({ t, transactions, categories, categoryIdByName, refreshCardAndTransactions, walletsList, user, cardBalance, setCardBalance, cardActivity, addActivity, cardFrozen, setCardFrozen, cardControls, setCardControls, dailyLimit, setDailyLimit, kycVerified, setKycVerified, tr }) {
  ...
  // The same generic sheets used everywhere else — undefined resolve/pay
  // props mean "use the main card", so one set of instances covers both.
  const lipaResolve = isMain ? undefined : (dest) => api.virtualCards.resolveLipa(selectedCardId, dest);
  const lipaPay = isMain ? undefined : (dest, amt, catId) => api.virtualCards.payLipa(selectedCardId, dest, amt, catId);
  ...
  return (
    <div className="px-4 pt-2 pb-8">
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 mb-4 -mx-4 px-4 snap-x snap-mandatory" style={{ scrollbarWidth: "none" }}>
        <PayCardTile t={t} selected={isMain} onSelect={() => setSelectedCardId("main")} label={holderName} sublabel="Primary card" balance={cardBalance} gradientFrom={t.accent} gradientTo={t.good} frozen={cardFrozen} />
        {virtualCards.map((c) => (
          <PayCardTile
            key={c.id} t={t} selected={selectedCardId === c.id} onSelect={() => setSelectedCardId(c.id)}
            label={c.label || `${nameFor(c.holderId)}'s card`} sublabel={nameFor(c.holderId)}
            badge={c.terminated ? "Terminated" : "Shared"} balance={c.balance}
            gradientFrom={t.gold} gradientTo="#A9782F" frozen={c.frozen} terminated={c.terminated}
          />
        ))}
      </div>
      ...
      <LipaSheet t={t} open={sheet === "lipa"} onClose={() => setSheet(null)} categoryIdByName={categoryIdByName} refreshCardAndTransactions={isMain ? refreshCardAndTransactions : undefined} kycVerified={kycVerified} setKycVerified={setKycVerified} resolveFn={lipaResolve} payFn={lipaPay} onSuccess={!isMain ? refreshSelectedVc : undefined} />
...
function VirtualCardDetailSheet({ t, card, onClose, categories, myUserId, holderName, onChanged, onMainCardChanged, onOpenService }) {
  ...
  const [revealed, setRevealed] = useState(null); // { fullNumber, cvv, expiry } once "Show card details" is used
  ...
  useEffect(() => {
    ...
    setTopupProvider(""); setTopupSpendAmt(""); setRevealed(null);
    const defaultCategory = categories.find((c) => c.id)?.id || "";
  ...
  const toggleReveal = async () => {
    ...
    try { setRevealed(await api.virtualCards.reveal(card.id)); }
    catch (err) { setError(err.message || "Couldn't load card details."); }
    finally { setRevealing(false); }
  };
  const formattedPan = revealed ? revealed.fullNumber.replace(/(.{4})/g, "$1 ").trim() : null;
  ...
  return (
    <BottomSheet t={t} open={!!card} onClose={onClose} title={card.label || (card.type === "parent_linked" ? (canManage ? `${holderName || "Member"}'s card` : "Your linked card") : "Independent card")}>
      <div className="p-4 rounded-[18px] mb-4" style={{ background: `linear-gradient(135deg, ${card.type === "parent_linked" ? t.gold : t.accent}, ${card.type === "parent_linked" ? "#A9782F" : t.good})` }}>
        ...
        <p className="text-[20px] font-bold" style={{ color: "#fff", fontFamily: "'IBM Plex Mono', monospace" }}>{formattedPan || `•••• •••• •••• ${card.last4}`}</p>
        {revealed && <p className="text-[12px] mt-1" style={{ color: "rgba(255,255,255,0.85)" }}>Exp {revealed.expiry} · CVV {revealed.cvv}</p>}
        <div className="flex items-center justify-between mt-3">
      ...
      {!card.terminated && (canManage || canSpend) && (
        <button onClick={toggleReveal} disabled={revealing} className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-[13px] text-[12.5px] font-semibold mb-4" style={{ background: t.cardSoft, border: `1px solid ${t.border}`, color: t.inkSoft }}>
          {revealing ? <RefreshCw size={13} className="animate-spin" /> : revealed ? <EyeOff size={13} /> : <Eye size={13} />}
          {revealing ? "Loading…" : revealed ? "Hide card details" : "Show full card number for online payment"}
        </button>
      )}
