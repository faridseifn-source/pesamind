...
import {
  ...
  Search, Calendar, Download, Pencil, CheckSquare, Square, Tag, ChevronUp,
} from "lucide-react";
import {
...
function OnboardingSlides({ t, onDone, onSkip }) {
...
function Onboarding({ t, onFinish, submitting, error }) {
  const [stage, setStage] = useState("slides");
  const [phone, setPhone] = useState("");
  return (
    <div style={{ background: t.bg }}>
      {stage === "slides" && <OnboardingSlides t={t} onDone={() => setStage("phone")} onSkip={() => setStage("phone")} />}
...
function ProfilePage({ t, user, avatarUrl, setAvatarUrl, goBack, tr }) {
  const fileRef = useRef(null);
  const handleFile = (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
  ...
  return (
    <div className="px-4 pt-2 pb-8">
      ...
      <Card t={t} className="p-4 space-y-3.5">
    ...
    return (
      <div style={{ background: t.bg, minHeight: "100vh", fontFamily: "'Inter', sans-serif" }}>
        ...
        <div className="max-w-[430px] mx-auto relative" style={{ background: t.bg, minHeight: "100vh" }}>
          <Onboarding t={t} onFinish={handleRegister} submitting={authSubmitting} error={authError} />
        </div>
      </div>
    );
  }
    ...
    <div style={{ background: t.bgSoft, minHeight: "100vh", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="max-w-[430px] mx-auto relative" style={{ background: t.bg, minHeight: "100vh" }}>
        ...
        <div style={{ paddingBottom: 84 }}>
          ...
          {active === "profile" && <ProfilePage t={t} user={user} avatarUrl={avatarUrl} setAvatarUrl={setAvatarUrl} goBack={() => setActive(profileReturnTab)} tr={tr} />}
