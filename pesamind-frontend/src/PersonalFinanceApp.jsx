...
function WalletsPage({ t, transactions, onAdd, walletsList, user, myInvites, onAcceptInvite, onDeclineInvite, onCreateSharedWallet, onRefreshWallets, setActive, categories, onAddCategory, onAddSubcategory, tr }) {
  ...
  return (
    <div className="px-4 pt-2 pb-24 relative">
      ...
      {!hasShared ? (
        ...
        <Card t={t} className="p-4 mb-4">
          <div className="flex items-center justify-between mb-2">
            ...
            {isOwner && <button onClick={() => setInviteOpen(true)} className="p-2 rounded-full" style={{ background: t.goldSoft }}><UserPlus size={15} color={t.gold} /></button>}
          </div>
        </Card>
      )}
      ...
      {sharedWallet && <InviteSheet t={t} open={inviteOpen} onClose={() => setInviteOpen(false)} walletId={sharedWallet.id} onInvited={onRefreshWallets} />}
    </div>
  );
}
...
function VirtualCardDetailSheet({ t, card, onClose, categories, myUserId, holderName, onChanged }) {
  ...
  const [activeService, setActiveService] = useState(null); // which service menu the holder is currently using
  const [spendAmt, setSpendAmt] = useState("");
  const [spendMerchant, setSpendMerchant] = useState("");
  const [spendCategory, setSpendCategory] = useState("");
  const [spendExtra, setSpendExtra] = useState(""); // meter/control number, when relevant
  ...
  useEffect(() => {
    ...
    setSpendAmt(""); setSpendMerchant(""); setSpendCategory(""); setSpendExtra("");
  ...
  const spentThisMonth = activity
    .filter((a) => a.type === "spend" && new Date(a.date).getMonth() === new Date().getMonth() && new Date(a.date).getFullYear() === new Date().getFullYear())
    .reduce((sum, a) => sum + (a.amount || 0), 0);

  const openService = (key, defaultCategory) => {
    setActiveService(key);
    setSpendAmt(""); setSpendMerchant(""); setSpendExtra(""); setError("");
    const match = categories.find((c) => c.id && c.name === defaultCategory);
    setSpendCategory(match?.id || categories.find((c) => c.id)?.id || "");
  };

  const submitSpend = async () => {
    if (!spendAmt || !spendCategory) return;
    const merchant = activeService === "luku" ? `LUKU Meter ${spendExtra}` : activeService === "gepg" ? `GePG · ${spendExtra || "Bill"}` : spendMerchant || VIRTUAL_CARD_SERVICES.find((s) => s.key === activeService)?.label || "Purchase";
    const result = await run(() => api.virtualCards.spend(card.id, parseFloat(spendAmt), merchant, spendCategory, activeService || "other"));
    if (result) setActiveService(null);
  };
  ...
  return (
    <BottomSheet t={t} open={!!card} onClose={onClose} title={card.label || (card.type === "parent_linked" ? (canManage ? `${holderName || "Member"}'s card` : "Your linked card") : "Independent card")}>
      ...
      {canSpend && !card.terminated && (
        <Card t={t} className="p-4 mb-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-[12px] font-semibold" style={{ color: t.inkFaint }}>Spend this month</p>
            <p className="text-[13px] font-bold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(spentThisMonth)}</p>
          </div>
          {card.frozen ? (
            ...
            <div className="grid grid-cols-4 gap-2">
              {VIRTUAL_CARD_SERVICES.filter((s) => (card.services ? card.services[s.key] !== false : true)).map((s) => (
                <button key={s.key} onClick={() => openService(s.key, s.defaultCategory)} className="flex flex-col items-center gap-1.5 py-3 rounded-[14px]" style={{ background: t.bgSoft, border: `1px solid ${t.border}` }}>
            ...
            <div>
              ...
              {activeService === "luku" && <Field t={t} label="Meter number"><input style={inputStyle(t)} value={spendExtra} onChange={(e) => setSpendExtra(e.target.value.replace(/\D/g, ""))} placeholder="e.g. 01234567891" /></Field>}
              {activeService === "gepg" && <Field t={t} label="Control number"><input style={inputStyle(t)} value={spendExtra} onChange={(e) => setSpendExtra(e.target.value.replace(/\D/g, ""))} placeholder="e.g. 991234567" /></Field>}
              {(activeService === "lipa" || activeService === "topup") && <Field t={t} label={activeService === "topup" ? "Provider / number" : "Merchant"}><input style={inputStyle(t)} value={spendMerchant} onChange={(e) => setSpendMerchant(e.target.value)} placeholder={activeService === "topup" ? "e.g. Vodacom 0712 345 678" : "e.g. Java House"} /></Field>}
              <Field t={t} label="Amount"><input style={inputStyle(t)} type="number" value={spendAmt} onChange={(e) => setSpendAmt(e.target.value)} /></Field>
              <Field t={t} label="Category">
                <select style={inputStyle(t)} value={spendCategory} onChange={(e) => setSpendCategory(e.target.value)}>
                  {categories.filter((c) => c.id && (!card.allowedCategoryIds || card.allowedCategoryIds.includes(c.id))).map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
              </Field>
              <PrimaryButton t={t} disabled={!spendAmt || !spendCategory || busy} onClick={submitSpend}>{busy ? "Processing…" : "Pay"}</PrimaryButton>
            </div>
          )}
        </Card>
      )}
        ...
        <>
          <Card t={t} className="p-4 mb-3">
            ...
            <div className="flex gap-2 mb-3">
              ...
              <button disabled={!topupAmt || busy} onClick={() => run(() => api.virtualCards.topup(card.id, parseFloat(topupAmt)))} className="px-4 rounded-[13px] font-semibold text-[13px]" style={{ background: t.accent, color: "#fff" }}>Send</button>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <button disabled={!transferAmt || busy} onClick={() => run(() => api.virtualCards.transferToMain(card.id, parseFloat(transferAmt)))} className="px-4 rounded-[13px] font-semibold text-[13px]" style={{ background: t.cardSoft, border: `1px solid ${t.border}`, color: t.ink }}>Send</button>
            </div>
          </Card>
...
function VirtualCardsPage({ t, goBack, categories, walletsList, user, tr }) {
    ...
    <div style={{ background: t.bgSoft, minHeight: "100vh", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="max-w-[430px] mx-auto relative" style={{ background: t.bg, minHeight: "100vh" }}>
        ...
        <div style={{ paddingBottom: 84 }}>
          ...
          {active === "virtualcards" && <VirtualCardsPage t={t} goBack={() => setActive("wallets")} categories={categories} walletsList={walletsList} user={user} tr={tr} />}
