...
function vatLabelFor(taxTreatment) {
  if (taxTreatment === "VAT_INCLUSIVE") return "VAT (inclusive)";
...
function getFeeBreakdown(fee, amount) {
  ...
  if (fee.totalFee !== undefined) {
    // A real, authoritative quote — the amount was already known.
    return {
      pesaMindFee: fee.pesaMindFee || 0, partnerFee: fee.partnerFee || 0, tax: fee.tax || 0, totalFee: fee.totalFee || 0,
      transfersEnabled: fee.transfersEnabled !== false, coveredMessage: null,
      wouldBePesaMindFee: fee.bundleBenefit > 0 ? fee.bundleBenefit : null,
      taxLabel: fee.tax > 0 ? vatLabelFor(fee.taxTreatment) : null,
    };
  }
  ...
  if (fee.coveredMessage) {
    ...
    return {
      pesaMindFee: 0, partnerFee, tax: 0, totalFee: partnerFee, transfersEnabled: fee.transfersEnabled !== false, coveredMessage: fee.coveredMessage,
      wouldBePesaMindFee: wouldBePesaMindFee > 0 ? wouldBePesaMindFee : null,
      taxLabel: fee.pesaMindFeeRule?.taxTreatment ? vatLabelFor(fee.pesaMindFeeRule.taxTreatment) : null,
    };
  }
  return {
      ...
      <PaySuccess
        ...
        extra={hasFee && (
          <>
            ...
            {payment.taxAmount > 0 && <div className="flex justify-between py-1.5" style={{ borderTop: `1px solid ${t.border}` }}><span className="text-[11.5px]" style={{ color: t.inkFaint }}>VAT</span><span className="text-[11.5px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(payment.taxAmount)}</span></div>}
            ...
            return (
              <div className="p-3.5 rounded-[14px] mb-4" style={{ background: t.bgSoft, border: `1px solid ${t.border}` }}>
                {!breakdown.transfersEnabled ? (
                  ...
                  <>
                    {breakdown.coveredMessage && <p className="text-[12px] font-semibold mb-2" style={{ color: t.good }}>{breakdown.coveredMessage}</p>}
                    ...
                    {breakdown.wouldBePesaMindFee > 0 ? (
                      <div className="flex justify-between mb-1">
                        <span className="text-[12px]" style={{ color: t.inkFaint }}>PesaMind fee</span>
                        <span className="text-[12px] font-semibold" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
                          <span style={{ textDecoration: "line-through", color: t.inkFaint, marginRight: 6 }}>{fmtTZS(breakdown.wouldBePesaMindFee)}</span>
                          <span style={{ color: t.good }}>Waived</span>
                        </span>
                      </div>
                    ) : breakdown.pesaMindFee > 0 && (
                      <div className="flex justify-between mb-1"><span className="text-[12px]" style={{ color: t.inkFaint }}>PesaMind fee</span><span className="text-[12px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(breakdown.pesaMindFee)}</span></div>
                    )}
                    ...
                    {breakdown.tax > 0 && <div className="flex justify-between mb-1"><span className="text-[12px]" style={{ color: t.inkFaint }}>{breakdown.taxLabel}</span><span className="text-[12px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(breakdown.tax)}</span></div>}
