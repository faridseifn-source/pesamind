...
function ChargesLine({ t, view, tr }) {
  ...
  const captionParts = [];
  if (view.charge > 0) captionParts.push(`${tr("feeCharge")} ${fmtTZS(view.charge)}`);
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-[12px] font-semibold" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
          {view.isWaived && view.wouldBeTotalCharges > 0 ? (
            ...
            <span style={{ color: t.ink }}>{view.totalCharges > 0 ? fmtTZS(view.totalCharges) : tr("feeFree")}</span>
          )}
        </span>
      </div>
      {view.coveredMessage ? (
...
function resizeImageForOcr(dataUrl, maxDimension = 1600, quality = 0.85) {
...
function ScanReceiptSheet({ t, open, onClose, onAdd, onUpdate, categories, tr }) {
  ...
  const startScan = async (photoData) => {
    setPhase("scanning");
    try {
      const resized = await resizeImageForOcr(photoData);
      ...
      onAdd({
        ...
        date: result.date || new Date().toISOString().slice(0, 10), wallet: "personal", loggedBy: "You", note: "Scanned from receipt", photo: photoData,
      });
      setPhase("result");
    } catch (err) {
            ...
            return (
              <div className="p-3.5 rounded-[14px] mb-4" style={{ background: t.bgSoft, border: `1px solid ${t.border}` }}>
                {!view.transfersEnabled ? (
                  ...
                  <>
                    <div className="flex justify-between mb-1"><span className="text-[12px]" style={{ color: t.inkFaint }}>{tr("feeAmount")}</span><span className="text-[12px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(amt)}</span></div>
                    <ChargesLine t={t} view={view} tr={tr} />
                    <div className="flex justify-between pt-1.5 mt-1" style={{ borderTop: `1px solid ${t.border}` }}><span className="text-[12.5px] font-semibold" style={{ color: t.ink }}>{tr("feeTotalAmount")}</span><span className="text-[13px] font-bold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(amt + view.totalCharges)}</span></div>
                  </>
                )}
              </div>
            );
...
function AdminSettingsTab({ t }) {
  ...
  const HELP = {
    ...
    receipt_ocr_enabled: "\"true\"/\"false\" — turns off receipt scanning immediately; customers are directed to add expenses manually instead. Which OCR provider actually runs (mock vs. real) is set via OCR_PROVIDER, an environment variable, not here.",
  };
