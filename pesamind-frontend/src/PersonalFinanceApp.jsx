...
import {
  ...
  Search, Calendar, Download, Pencil, CheckSquare, Square, Tag, ChevronUp, LogOut,
} from "lucide-react";
import {
...
const TRANSLATIONS = {
  en: {
    ...
    pay: "Pay", paySub: "Your prepaid card, funded from your linked bank account.",
  ...
  sw: {
    ...
    pay: "Lipa", paySub: "Kadi yako ya kulipia, inayofadhiliwa na akaunti yako ya benki iliyounganishwa.",
...
function PhoneStage({ t, onBack, onSent, phone, setPhone }) {
  ...
  return (
    <div className="flex flex-col h-screen px-6 pt-6 pb-8">
      ...
      <button onClick={() => valid && onSent()} disabled={!valid} className="w-full py-3.5 rounded-full font-semibold text-[15px]" style={{ background: t.accent, color: "#fff", fontFamily: "'Space Grotesk', sans-serif", opacity: valid ? 1 : 0.5 }}>Send code</button>
    </div>
  );
}
...
function OtpStage({ t, onBack, onVerified, phone }) {
  ...
  const verify = () => { setVerifying(true); setTimeout(() => { setVerifying(false); onVerified(); }, 900); };
  ...
  return (
    <div className="flex flex-col h-screen px-6 pt-6 pb-8">
      ...
      <button onClick={() => setTimer(30)} disabled={timer > 0} className="text-[13px] font-medium text-left" style={{ color: timer > 0 ? t.inkFaint : t.accent }}>{timer > 0 ? `Resend code in ${timer}s` : "Resend code"}</button>
      <div className="flex-1" />
...
function LoginStage({ t, onBack, onDone, onForgotPassword, submitting, error }) {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
...
function Onboarding({ t, onFinish, submitting, error, onLogin, loginSubmitting, loginError, onForgotPassword, forgotSubmitting, forgotError, forgotMessage, onResetPassword, resetSubmitting, resetError, resetMessage }) {
  ...
  return (
    <div style={{ background: t.bg }}>
      ...
      {stage === "otp" && <OtpStage t={t} phone={phone} onBack={() => setStage("phone")} onVerified={() => setStage("profile")} />}
      {stage === "profile" && <ProfileStage t={t} onBack={() => setStage("otp")} onDone={(profile) => onFinish({ ...profile, phone })} submitting={submitting} error={error} />}
      {stage === "login" && <LoginStage t={t} onBack={() => setStage("slides")} onDone={onLogin} onForgotPassword={() => setStage("forgot")} submitting={loginSubmitting} error={loginError} />}
...
function HomePage({ t, transactions, budgets, categories, user, tr, setActive, onAdd, onUpdate, onAddCategory, onAddSubcategory, kycVerified, setKycVerified }) {
  ...
  return (
    <div className="px-4 pt-2 pb-6 space-y-3">
      ...
      <LipaSheet t={t} open={lipaOpen} onClose={() => setLipaOpen(false)} onComplete={onAdd} kycVerified={kycVerified} setKycVerified={setKycVerified} />
...
function LipaSheet({ t, open, onClose, onComplete, kycVerified, setKycVerified }) {
  ...
  useEffect(() => {
    if (open) {
      setPhase("scan"); setMerchant(""); setAmount(""); setTipsNumber(""); receiptId.current = shortId();
      const timer = setTimeout(() => {
        setPhase((prev) => {
          if (prev !== "scan") return prev;
          setMerchant(QR_MERCHANTS[Math.floor(Math.random() * QR_MERCHANTS.length)]);
          setAmount((8 + Math.random() * 40).toFixed(2));
          return "confirm";
        });
      }, 1400);
  ...
  const lookUpTips = () => { if (!tipsNumber || !amount) return; setMerchant(resolveTips(tipsNumber)); setPhase("confirm"); };
  const amt = parseFloat(amount) || 0;
  const pay = () => { if (amt > KYC_THRESHOLD && !kycVerified) setPhase("kyc"); else setPhase("success"); };
  const finish = () => { onComplete({ id: uid(), amount: -Math.abs(parseFloat(amount) || 0), merchant, category: "Shopping", date: "2026-07-30", wallet: "personal", loggedBy: "You" }); onClose(); };
      ...
      {phase === "confirm" && (
        <div>
          <div className="flex items-center gap-3 p-4 rounded-[16px] mb-4" style={{ background: t.cardSoft, border: `1px solid ${t.border}` }}>
            ...
            <div><p className="text-[11.5px]" style={{ color: t.inkFaint }}>{tipsNumber ? "Resolved recipient" : "Paying"}</p><p className="text-[14.5px] font-bold" style={{ color: t.ink }}>{merchant}</p>{tipsNumber && <p className="text-[11.5px]" style={{ color: t.inkFaint, fontFamily: "'IBM Plex Mono', monospace" }}>{tipsNumber}</p>}</div>
          </div>
          <Field t={t} label="Amount"><input style={inputStyle(t)} type="number" value={amount} onChange={(e) => setAmount(e.target.value)} /></Field>
          <PrimaryButton t={t} onClick={pay}>Pay {fmt(-Math.abs(amt))}</PrimaryButton>
        </div>
      )}
      {phase === "kyc" && <KycStep t={t} amount={amt} onVerified={() => { setKycVerified(true); setPhase("success"); }} />}
      {phase === "success" && <PaySuccess t={t} title="Payment successful" amount={parseFloat(amount) || 0} sublabel={`Paid to ${merchant}`} shareText={`PesaMind Receipt\nMerchant: ${merchant}\nAmount: ${fmt(-Math.abs(parseFloat(amount) || 0))}\nReceipt ID: ${receiptId.current}\nVerification code: ${verifyCode}`} receiptId={receiptId.current} verifyCode={verifyCode} onDone={finish} />}
    </BottomSheet>
  );
}

function GepgPaySheet({ t, open, onClose, onComplete, kycVerified, setKycVerified }) {
  const [step, setStep] = useState(0);
  const [control, setControl] = useState("");
  const [bill, setBill] = useState(null);
  const receiptId = useRef(shortId());

  const reset = () => { setStep(0); setControl(""); setBill(null); };
  const checkBill = () => { if (control.length < 6) return; setBill(resolveGepg(control)); receiptId.current = shortId(); setStep(1); };
  const pay = () => { if (bill.amount > KYC_THRESHOLD && !kycVerified) setStep("kyc"); else setStep(2); };
  const verifyCode = `VC-${Math.abs(hashCode(receiptId.current)).toString(16).toUpperCase().slice(0, 6)}`;
  const finish = () => { onComplete({ id: uid(), amount: -Math.abs(bill.amount), merchant: bill.biller, category: "Other", date: "2026-07-30", wallet: "personal", loggedBy: "You" }); onClose(); reset(); };
    ...
    <BottomSheet t={t} open={open} onClose={() => { onClose(); reset(); }} title="Pay government · GePG" onBack={step === 1 ? () => setStep(0) : undefined}>
      ...
      {step === 1 && bill && (
        <div>
          ...
          <PrimaryButton t={t} onClick={pay} tone={t.gold}>Pay now</PrimaryButton>
        </div>
      )}
      {step === "kyc" && bill && <KycStep t={t} amount={bill.amount} onVerified={() => { setKycVerified(true); setStep(2); }} />}
...
function LukuPaySheet({ t, open, onClose, onComplete, kycVerified, setKycVerified }) {
  const [step, setStep] = useState(0);
  ...
  const reset = () => { setStep(0); setMeter(""); setAmount(""); setToken(""); };
  ...
  const finalizePurchase = () => { setToken(genLukuToken()); receiptId.current = shortId(); setStep(2); };
  const buy = () => { if (amt > KYC_THRESHOLD && !kycVerified) setStep("kyc"); else finalizePurchase(); };
  const verifyCode = `VC-${Math.abs(hashCode(receiptId.current)).toString(16).toUpperCase().slice(0, 6)}`;
  const finish = () => { onComplete({ id: uid(), amount: -Math.abs(parseFloat(amount) || 0), merchant: `LUKU · Meter ${meter}`, category: "Housing & Utilities", date: "2026-07-30", wallet: "personal", loggedBy: "You" }); onClose(); reset(); };
    ...
    <BottomSheet t={t} open={open} onClose={() => { onClose(); reset(); }} title="Pay electricity · LUKU" onBack={step === 1 ? () => setStep(0) : undefined}>
      ...
      {step === 1 && (
        <div>
          ...
          <PrimaryButton t={t} onClick={buy} tone="#4A8C8C">Confirm & buy token</PrimaryButton>
        </div>
      )}
      {step === "kyc" && <KycStep t={t} amount={amt} onVerified={() => { setKycVerified(true); finalizePurchase(); }} />}
      {step === 2 && (
        <PaySuccess t={t} icon={Zap} title="Token generated" amount={parseFloat(amount) || 0} sublabel={`${units} kWh · Meter ${meter}`}
...
function TopUpGatewaySheet({ t, open, onClose, cardBalance, setCardBalance, addActivity, kycVerified, setKycVerified }) {
  const [step, setStep] = useState(0);
  ...
  const reset = () => { setStep(0); setCardNumber(""); setExpiry(""); setCvv(""); setAmount(""); };
  ...
  const finalize = () => {
    ...
    addActivity({ id: uid(), type: "topup", amount: amt, label: "Card gateway top-up", sub: `•••• ${digits.slice(-4)}`, date: "2026-07-30" });
    setStep(2);
  };
    ...
    <BottomSheet t={t} open={open} onClose={() => { onClose(); reset(); }} title="Card payment gateway" onBack={step === 1 ? () => setStep(0) : undefined}>
      ...
      {step === 1 && (
        <div>
          ...
          <PrimaryButton t={t} onClick={confirm}>Load funds</PrimaryButton>
        </div>
      )}
      {step === "kyc" && <KycStep t={t} amount={amt} onVerified={() => { setKycVerified(true); finalize(); }} />}
      {step === 2 && <CardSuccess t={t} icon={ArrowDownCircle} title="Funds added" amountLabel={`+${fmtTZS(amt)}`} amountColor={t.good} sublabel={`New balance ${fmtTZS(cardBalance + amt)}`} onDone={() => { onClose(); reset(); }} />}
    </BottomSheet>
  );
}

function VisaOctSheet({ t, open, onClose, cardBalance, setCardBalance, addActivity, kycVerified, setKycVerified }) {
  const [step, setStep] = useState(0);
  const [cardNumber, setCardNumber] = useState("");
  const [amount, setAmount] = useState("");
  const reset = () => { setStep(0); setCardNumber(""); setAmount(""); };
  const digits = cardNumber.replace(/\D/g, "");
  const valid = digits.length === 16 && parseFloat(amount) > 0;
  const amt = parseFloat(amount) || 0;

  const finalize = () => { setCardBalance((b) => b + amt); addActivity({ id: uid(), type: "oct", amount: amt, label: "Visa OCT push", sub: `•••• ${digits.slice(-4)}`, date: "2026-07-30" }); setStep(2); };
  const confirm = () => { if (amt > KYC_THRESHOLD && !kycVerified) setStep("kyc"); else finalize(); };
    ...
    <BottomSheet t={t} open={open} onClose={() => { onClose(); reset(); }} title="Visa OCT" onBack={step === 1 ? () => setStep(0) : undefined}>
      ...
      {step === 1 && (
        <div>
          ...
          <PrimaryButton t={t} onClick={confirm} tone="#1A5C97">Pull funds now</PrimaryButton>
        </div>
      )}
      {step === "kyc" && <KycStep t={t} amount={amt} onVerified={() => { setKycVerified(true); finalize(); }} />}
      {step === 2 && <CardSuccess t={t} icon={ArrowDownCircle} title="Funds pulled" amountLabel={`+${fmtTZS(amt)}`} amountColor={t.good} sublabel={`New balance ${fmtTZS(cardBalance + amt)}`} onDone={() => { onClose(); reset(); }} />}
    </BottomSheet>
  );
}
...
function PayPage({ t, transactions, onAdd, categories, user, cardBalance, setCardBalance, cardActivity, addActivity, cardFrozen, setCardFrozen, cardControls, setCardControls, dailyLimit, setDailyLimit, kycVerified, setKycVerified, tr }) {
  ...
  return (
    <div className="px-4 pt-2 pb-8">
      ...
      <p className="text-[13px] mb-1" style={{ color: t.inkFaint }}>{tr("paySub")}</p>

      <VirtualCard t={t} frozen={cardFrozen} holderName={holderName} revealed={revealed} onToggleReveal={() => setRevealed((r) => !r)} />
      ...
      <LipaSheet t={t} open={sheet === "lipa"} onClose={() => setSheet(null)} onComplete={onAdd} kycVerified={kycVerified} setKycVerified={setKycVerified} />
      <GepgPaySheet t={t} open={sheet === "gepg"} onClose={() => setSheet(null)} onComplete={onAdd} kycVerified={kycVerified} setKycVerified={setKycVerified} />
      <LukuPaySheet t={t} open={sheet === "luku"} onClose={() => setSheet(null)} onComplete={onAdd} kycVerified={kycVerified} setKycVerified={setKycVerified} />

      <TopUpMenuSheet t={t} open={sheet === "topup-menu"} onClose={() => setSheet(null)} onNavigate={setSheet} />
      <TopUpGatewaySheet t={t} open={sheet === "topup-gateway"} onClose={() => setSheet(null)} cardBalance={cardBalance} setCardBalance={setCardBalance} addActivity={addActivity} kycVerified={kycVerified} setKycVerified={setKycVerified} />
      <VisaOctSheet t={t} open={sheet === "topup-oct"} onClose={() => setSheet(null)} cardBalance={cardBalance} setCardBalance={setCardBalance} addActivity={addActivity} kycVerified={kycVerified} setKycVerified={setKycVerified} />
...
function InviteSheet({ t, open, onClose }) {
  const [code] = useState("HH-7F3K-92QX");
  const [copied, setCopied] = useState(false);
  const [email, setEmail] = useState("");
  ...
  return (
    <BottomSheet t={t} open={open} onClose={onClose} title="Link a family member">
      <p className="text-[13px] mb-4" style={{ color: t.inkFaint }}>Share a join code or send an email invite to link another account to your Shared Household Wallet.</p>
      <p className="text-[12px] font-semibold mb-1.5" style={{ color: t.inkFaint }}>Share a join code</p>
      <div className="flex items-center justify-between p-4 rounded-[16px] mb-5" style={{ background: t.cardSoft, border: `1px dashed ${t.border}` }}>
        <span className="text-[19px] font-bold tracking-widest" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{code}</span>
        <button onClick={copy} className="p-2 rounded-full" style={{ background: t.card, border: `1px solid ${t.border}` }}>{copied ? <Check size={16} color={t.good} /> : <Copy size={16} color={t.inkSoft} />}</button>
      </div>
      <div className="flex items-center gap-3 mb-5"><div className="h-px flex-1" style={{ background: t.border }} /><span className="text-[11px] uppercase font-semibold" style={{ color: t.inkFaint }}>or</span><div className="h-px flex-1" style={{ background: t.border }} /></div>
      <Field t={t} label="Partner's email address"><input style={inputStyle(t)} type="email" placeholder="partner@email.com" value={email} onChange={(e) => setEmail(e.target.value)} /></Field>
      <PrimaryButton t={t} onClick={() => email && setSent(true)} tone={sent ? t.accentSoft : t.accent}>{sent ? <><Check size={16} /> Invite sent to {email}</> : "Send invitation"}</PrimaryButton>
    </BottomSheet>
  );
}

function WalletsPage({ t, transactions, onAdd, sharedEnabled, setSharedEnabled, categories, onAddCategory, onAddSubcategory, tr }) {
  const [view, setView] = useState("personal");
  const [sheetOpen, setSheetOpen] = useState(false);
  const [inviteOpen, setInviteOpen] = useState(false);
  const toggleShared = (next) => { setSharedEnabled(next); if (!next) setView("personal"); else setInviteOpen(true); };
  const activeWallet = sharedEnabled ? view : "personal";
  ...
  return (
    <div className="px-4 pt-2 pb-24 relative">
      ...
      <Card t={t} className="p-4 mb-4 flex items-center justify-between">
        <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: sharedEnabled ? t.accentSoft : t.bgSoft }}><Users size={16} color={sharedEnabled ? t.good : t.inkFaint} /></div><div><p className="text-[13.5px] font-semibold" style={{ color: t.ink }}>{tr("sharedHouseholdWallet")}</p><p className="text-[11.5px]" style={{ color: t.inkFaint }}>{sharedEnabled ? "On · linked with Alex & Jordan" : "Off · your spending stays private"}</p></div></div>
        <Switch t={t} on={sharedEnabled} onChange={toggleShared} />
      </Card>
      {sharedEnabled && (
        <div className="flex p-1 rounded-full mb-4" style={{ background: t.bgSoft, border: `1px solid ${t.border}` }}>
      ...
      {sharedEnabled && activeWallet === "shared" && (
        <button onClick={() => setInviteOpen(true)} className="w-full flex items-center justify-between p-4 rounded-[18px] mb-4" style={{ background: t.goldSoft, border: `1px solid ${t.border}` }}>
          <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: t.card }}><UserPlus size={16} color={t.gold} /></div><div className="text-left"><p className="text-[13.5px] font-semibold" style={{ color: t.ink }}>Link another family member</p><p className="text-[11.5px]" style={{ color: t.inkFaint }}>Grow the household wallet</p></div></div>
      ...
      <AddTxSheet t={t} open={sheetOpen} onClose={() => setSheetOpen(false)} onAdd={onAdd} wallet={activeWallet} loggedByOptions={activeWallet === "shared" ? ["You", "Alex", "Jordan"] : undefined} categories={categories} onAddCategory={onAddCategory} onAddSubcategory={onAddSubcategory} />
      <InviteSheet t={t} open={inviteOpen} onClose={() => setInviteOpen(false)} />
    </div>
  );
}
...
function ProfilePage({ t, user, avatarUrl, setAvatarUrl, goBack, tr, onLogout }) {
  const fileRef = useRef(null);
  const handleFile = (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
...
export default function App() {
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      const [txs, kycStatus, cardData, remoteBudgets] = await Promise.all([
    ...
    <div style={{ background: t.bgSoft, minHeight: "100vh", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="max-w-[430px] mx-auto relative" style={{ background: t.bg, minHeight: "100vh" }}>
        ...
        <div style={{ paddingBottom: 84 }}>
          {active === "home" && <HomePage t={t} transactions={transactions} budgets={budgets} categories={categories} user={user} tr={tr} setActive={setActive} onAdd={addTransaction} onUpdate={updateTransaction} onAddCategory={addCategory} onAddSubcategory={addSubcategory} kycVerified={kycVerified} setKycVerified={setKycVerified} />}
          ...
          {active === "pay" && (
            <PayPage t={t} transactions={transactions} onAdd={addTransaction} categories={categories} user={user}
              cardBalance={cardBalance} setCardBalance={setCardBalance} cardActivity={cardActivity} addActivity={addCardActivity}
          ...
          {active === "wallets" && <WalletsPage t={t} transactions={transactions} onAdd={addTransaction} sharedEnabled={sharedEnabled} setSharedEnabled={setSharedEnabled} categories={categories} onAddCategory={addCategory} onAddSubcategory={addSubcategory} tr={tr} />}
          {active === "profile" && <ProfilePage t={t} user={user} avatarUrl={avatarUrl} setAvatarUrl={setAvatarUrl} goBack={() => setActive(profileReturnTab)} tr={tr} onLogout={handleLogout} />}
