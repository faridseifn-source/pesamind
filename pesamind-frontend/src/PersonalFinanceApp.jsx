...
import {
  ...
  Search, Calendar, Download, Pencil, CheckSquare, Square, Tag, ChevronUp, LogOut, FileText, MessageSquareWarning, Package,
} from "lucide-react";
import {
...
function getFeeBreakdown(fee, amount) {
  if (!fee) return { pesaMindFee: 0, partnerFee: 0, tax: 0, totalFee: 0, transfersEnabled: true, coveredMessage: null };
  if (fee.totalFee !== undefined) {
    return { pesaMindFee: fee.pesaMindFee || 0, partnerFee: fee.partnerFee || 0, tax: fee.tax || 0, totalFee: fee.totalFee || 0, transfersEnabled: fee.transfersEnabled !== false, coveredMessage: null };
  }
  if (fee.coveredMessage) {
    // Bundle/exemption covers this transaction type — the PesaMind fee is
    // genuinely 0, not just "not estimated yet." The partner/institution
    // fee (a bank markup, separate from PesaMind's own fee) still applies
    // independently, since a bundle only ever covers PesaMind's own fee.
    const partnerFee = computeFeeEstimate(fee.partnerFee, amount);
    return { pesaMindFee: 0, partnerFee, tax: 0, totalFee: partnerFee, transfersEnabled: fee.transfersEnabled !== false, coveredMessage: fee.coveredMessage };
  }
  const pesaMindFee = computePesaMindFeeEstimate(fee.pesaMindFeeRule, amount);
  const partnerFee = computeFeeEstimate(fee.partnerFee, amount);
  return { pesaMindFee, partnerFee, tax: 0, totalFee: pesaMindFee + partnerFee, transfersEnabled: fee.transfersEnabled !== false, coveredMessage: null };
}
...
function HomePage({ t, transactions, budgets, categories, categoryIdByName, refreshCardAndTransactions, user, tr, setActive, onAdd, onUpdate, onAddCategory, onAddSubcategory, kycVerified, setKycVerified }) {
  ...
  return (
    <div className="px-4 pt-2 pb-6 space-y-3">
      ...
      <QrPaymentSheet t={t} open={qrPayOpen} onClose={() => setQrPayOpen(false)} cardType="main" cardId="main" categories={categories} kycVerified={kycVerified} setKycVerified={setKycVerified} onSuccess={refreshCardAndTransactions} />
...
function QrPaymentResult({ t, payment, merchant, onDone, onRetry }) {
  ...
  if (success) {
    ...
    const totalCharges = Number(payment.feeAmount || 0) + Number(payment.pesaMindFeeAmount || 0) + Number(payment.taxAmount || 0);
    ...
    return (
      <PaySuccess
        ...
        extra={hasFee && (
          <>
            {payment.pesaMindFeeAmount > 0 && <div className="flex justify-between py-1.5" style={{ borderTop: `1px solid ${t.border}` }}><span className="text-[11.5px]" style={{ color: t.inkFaint }}>PesaMind fee</span><span className="text-[11.5px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(payment.pesaMindFeeAmount)}</span></div>}
            {payment.feeAmount > 0 && <div className="flex justify-between py-1.5" style={{ borderTop: `1px solid ${t.border}` }}><span className="text-[11.5px]" style={{ color: t.inkFaint }}>Partner/bank fee</span><span className="text-[11.5px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(payment.feeAmount)}</span></div>}
            {payment.taxAmount > 0 && <div className="flex justify-between py-1.5" style={{ borderTop: `1px solid ${t.border}` }}><span className="text-[11.5px]" style={{ color: t.inkFaint }}>Tax</span><span className="text-[11.5px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(payment.taxAmount)}</span></div>}
            <div className="flex justify-between py-1.5" style={{ borderTop: `1px solid ${t.border}` }}>
    ...
    <BottomSheet t={t} open={open} onClose={onClose} title="Lipa">
      ...
      {phase === "confirm" && resolved && (
        <div>
          ...
          {resolved.fee && amt > 0 && (() => {
            const breakdown = getFeeBreakdown(resolved.fee, amt);
            return (
              <div className="p-3.5 rounded-[14px] mb-4" style={{ background: t.bgSoft, border: `1px solid ${t.border}` }}>
                {!breakdown.transfersEnabled ? (
                  <p className="text-[12px]" style={{ color: t.danger }}>Transfers to this institution are currently unavailable.</p>
                ) : breakdown.coveredMessage ? (
                  <>
                    <p className="text-[12px] font-semibold mb-1.5" style={{ color: t.good }}>{breakdown.coveredMessage}</p>
                    {breakdown.partnerFee > 0 && <div className="flex justify-between mb-1"><span className="text-[12px]" style={{ color: t.inkFaint }}>Partner/bank fee</span><span className="text-[12px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(breakdown.partnerFee)}</span></div>}
                    <div className="flex justify-between pt-1.5" style={{ borderTop: `1px solid ${t.border}` }}><span className="text-[12.5px] font-semibold" style={{ color: t.ink }}>Total to pay</span><span className="text-[13px] font-bold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(amt + breakdown.totalFee)}</span></div>
                  </>
                ) : (
                  <>
                    <div className="flex justify-between mb-1"><span className="text-[12px]" style={{ color: t.inkFaint }}>Amount</span><span className="text-[12px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(amt)}</span></div>
                    {breakdown.pesaMindFee > 0 && <div className="flex justify-between mb-1"><span className="text-[12px]" style={{ color: t.inkFaint }}>PesaMind fee</span><span className="text-[12px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(breakdown.pesaMindFee)}</span></div>}
                    {breakdown.partnerFee > 0 && <div className="flex justify-between mb-1"><span className="text-[12px]" style={{ color: t.inkFaint }}>Partner/bank fee</span><span className="text-[12px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(breakdown.partnerFee)}</span></div>}
                    {breakdown.tax > 0 && <div className="flex justify-between mb-1"><span className="text-[12px]" style={{ color: t.inkFaint }}>Tax</span><span className="text-[12px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(breakdown.tax)}</span></div>}
                    <div className="flex justify-between mb-1"><span className="text-[12px]" style={{ color: t.inkFaint }}>Total charges</span><span className="text-[12px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{breakdown.totalFee > 0 ? fmtTZS(breakdown.totalFee) : "FREE"}</span></div>
                    <div className="flex justify-between pt-1.5" style={{ borderTop: `1px solid ${t.border}` }}><span className="text-[12.5px] font-semibold" style={{ color: t.ink }}>Total to pay</span><span className="text-[13px] font-bold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(amt + breakdown.totalFee)}</span></div>
                  </>
                )}
              </div>
          ...
          {error && <p className="text-[12.5px] mb-3" style={{ color: t.danger }}>{error}</p>}
          <PrimaryButton t={t} onClick={goToAuth} disabled={resolved.fee && !getFeeBreakdown(resolved.fee, amt).transfersEnabled}>Continue</PrimaryButton>
        </div>
      )}
                ...
                return (
                  <div key={b.id} className="p-3.5 rounded-[16px]" style={{ background: t.cardSoft, border: `1px solid ${t.border}` }}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <p className="text-[14px] font-bold" style={{ color: t.accent, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(b.price)}</p>
                    </div>
                    <p className="text-[11.5px] mb-2" style={{ color: t.inkFaint }}>Per {validityLabel[b.validity]}{b.includedTransactionCount ? ` · ${b.includedTransactionCount} transactions included` : " · Unlimited eligible transactions"}</p>
