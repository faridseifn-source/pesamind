...
function PayPage({ t, transactions, categories, categoryIdByName, refreshCardAndTransactions, walletsList, user, cardBalance, setCardBalance, cardLast4, cardActivity, addActivity, cardFrozen, setCardFrozen, cardControls, setCardControls, dailyLimit, setDailyLimit, kycVerified, setKycVerified, tr }) {
  ...
  const [cardRevealed, setCardRevealed] = useState(null); // { fullNumber, cvv, expiry } once "reveal" is used
  ...
  const toggleCardReveal = async () => {
    if (cardRevealed) { setCardRevealed(null); return; }
    setCardRevealing(true);
    try { setCardRevealed(await api.cards.reveal()); }
    catch (err) { console.error("Failed to reveal card details", err); } // eslint-disable-line no-console
    finally { setCardRevealing(false); }
  };
    ...
    <div className="px-4 pt-2 pb-8">
      ...
      >
        <div className="pt-2">
          {isMain && (
            <>
              <CardFace t={t} holderName={holderName} last4={cardLast4} revealed={cardRevealed} revealing={cardRevealing} onToggleReveal={toggleCardReveal} gradientFrom={t.accent} gradientTo={t.good} />
