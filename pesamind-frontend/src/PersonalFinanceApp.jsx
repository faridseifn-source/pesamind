...
function WalletCardStack({ t, cards, selectedId, onSelect, expanded, onExpandedChange, balanceHidden, onToggleBalanceHidden }) {
  const dragY = useMotionValue(0);

  const CARD_HEIGHT = 148;
  const PEEK = 16; // px of each card visible behind the one in front, collapsed
  const GAP = CARD_HEIGHT + 14; // vertical spacing between cards, expanded — must be >= CARD_HEIGHT or cards overlap and hide each other's top portion
            ...
            >
              ...
              {showDetails ? (
                <>
                  ...
                  <div className="flex items-center gap-2">
                    <p className="text-[18px] font-bold" style={{ color: c.terminated ? t.ink : "#fff", fontFamily: "'IBM Plex Mono', monospace" }}>
                      {balanceHidden ? "TZS •••••••" : fmtTZS(c.balance)}
                    </p>
                    <button
                    ...
                    >
                      {balanceHidden ? <EyeOff size={15} color={c.terminated ? t.inkFaint : "rgba(255,255,255,0.75)"} /> : <Eye size={15} color={c.terminated ? t.inkFaint : "rgba(255,255,255,0.75)"} />}
                    </button>
                  </div>
                </>
              ) : (
