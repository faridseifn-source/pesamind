...
function LipaSheet({ t, open, onClose, categoryIdByName, refreshCardAndTransactions, kycVerified, setKycVerified }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      const timer = setTimeout(async () => {
        ...
        try {
          const { recipient } = await api.cards.resolveLipa(till);
  ...
  const lookUpTips = async () => {
    ...
    setError("");
    try {
      const { recipient } = await api.cards.resolveLipa(tipsNumber);
      setMerchant(recipient.name);
      setPhase("confirm");
    } catch (err) {
  ...
  const finalizePay = async () => {
    ...
    try {
      ...
      await api.cards.payLipa(tipsNumber, amt, categoryId);
  ...
  const pay = () => { if (amt > KYC_THRESHOLD && !kycVerified) setPhase("kyc"); else finalizePay(); };
    ...
    <BottomSheet t={t} open={open} onClose={onClose} title="Lipa" onBack={phase === "manual" ? () => setPhase("scan") : phase === "confirm" && tipsNumber ? () => setPhase("manual") : undefined}>
      ...
      {phase === "confirm" && (
        <div>
          ...
          <Field t={t} label="Amount"><input style={inputStyle(t)} type="number" value={amount} onChange={(e) => setAmount(e.target.value)} /></Field>
          {error && <p className="text-[12px] mb-3" style={{ color: t.danger }}>{error}</p>}
          <PrimaryButton t={t} onClick={pay} disabled={processing}>{processing ? "Paying…" : `Pay ${fmt(-Math.abs(amt))}`}</PrimaryButton>
        </div>
      )}
...
function GepgPaySheet({ t, open, onClose, categoryIdByName, refreshCardAndTransactions, kycVerified, setKycVerified }) {
  const [step, setStep] = useState(0);
  ...
  const finalizePay = async () => {
    ...
    try {
      ...
      await api.cards.payGepg(control, bill.amount, bill.biller, categoryId);
      await refreshCardAndTransactions();
      setStep(2);
    } catch (err) {
      setError(err.message || "Payment failed. Please try again.");
  ...
  const pay = () => { if (bill.amount > KYC_THRESHOLD && !kycVerified) setStep("kyc"); else finalizePay(); };
    ...
    <BottomSheet t={t} open={open} onClose={() => { onClose(); reset(); }} title="Pay government · GePG" onBack={step === 1 ? () => setStep(0) : undefined}>
      ...
      {step === 1 && bill && (
        <div>
          ...
          {error && <p className="text-[12px] mb-3" style={{ color: t.danger }}>{error}</p>}
          <PrimaryButton t={t} onClick={pay} tone={t.gold} disabled={processing}>{processing ? "Paying…" : "Pay now"}</PrimaryButton>
        </div>
      )}
...
function LukuPaySheet({ t, open, onClose, categoryIdByName, refreshCardAndTransactions, kycVerified, setKycVerified }) {
  const [step, setStep] = useState(0);
  ...
  const finalizePurchase = async () => {
    ...
    try {
      ...
      const { token: serverToken } = await api.cards.payLuku(meter, amt, categoryId);
    ...
    <BottomSheet t={t} open={open} onClose={() => { onClose(); reset(); }} title="Pay electricity · LUKU" onBack={step === 1 ? () => setStep(0) : undefined}>
      ...
      {step === 1 && (
        <div>
          ...
          {error && <p className="text-[12px] mb-3" style={{ color: t.danger }}>{error}</p>}
          <PrimaryButton t={t} onClick={buy} tone="#4A8C8C" disabled={processing}>{processing ? "Buying…" : "Confirm & buy token"}</PrimaryButton>
        </div>
      )}
...
function PayPage({ t, transactions, categories, categoryIdByName, refreshCardAndTransactions, user, cardBalance, setCardBalance, cardActivity, addActivity, cardFrozen, setCardFrozen, cardControls, setCardControls, dailyLimit, setDailyLimit, kycVerified, setKycVerified, tr }) {
  ...
  return (
    <div className="px-4 pt-2 pb-8">
      ...
      <LipaSheet t={t} open={sheet === "lipa"} onClose={() => setSheet(null)} categoryIdByName={categoryIdByName} refreshCardAndTransactions={refreshCardAndTransactions} kycVerified={kycVerified} setKycVerified={setKycVerified} />
      <GepgPaySheet t={t} open={sheet === "gepg"} onClose={() => setSheet(null)} categoryIdByName={categoryIdByName} refreshCardAndTransactions={refreshCardAndTransactions} kycVerified={kycVerified} setKycVerified={setKycVerified} />
      <LukuPaySheet t={t} open={sheet === "luku"} onClose={() => setSheet(null)} categoryIdByName={categoryIdByName} refreshCardAndTransactions={refreshCardAndTransactions} kycVerified={kycVerified} setKycVerified={setKycVerified} />
...
function VirtualCardDetailSheet({ t, card, onClose, categories, myUserId, holderName, onChanged, onMainCardChanged }) {
  ...
  const [lipaDest, setLipaDest] = useState("");
  const [lipaAmt, setLipaAmt] = useState("");
  const [lipaCategory, setLipaCategory] = useState("");
  const [gepgControl, setGepgControl] = useState("");
  const [gepgBill, setGepgBill] = useState(null);
  const [gepgCategory, setGepgCategory] = useState("");
  const [lukuMeter, setLukuMeter] = useState("");
  const [lukuAmt, setLukuAmt] = useState("");
  const [lukuCategory, setLukuCategory] = useState("");
  const [lukuToken, setLukuToken] = useState("");
  ...
  useEffect(() => {
    ...
    setLipaDest(""); setLipaAmt(""); setGepgControl(""); setGepgBill(null); setLukuMeter(""); setLukuAmt(""); setLukuToken(""); setTopupProvider(""); setTopupSpendAmt("");
    const defaultCategory = categories.find((c) => c.id)?.id || "";
    setLipaCategory(defaultCategory); setGepgCategory(defaultCategory); setLukuCategory(defaultCategory); setTopupCategory(defaultCategory);
  ...
  const checkGepgBill = () => { if (gepgControl.length < 6) return; setGepgBill(resolveGepg(gepgControl)); };

  const openService = (key) => { setActiveService(key); setError(""); };
          ...
          {card.frozen ? (
            ...
            <div className="grid grid-cols-4 gap-2">
              {VIRTUAL_CARD_SERVICES.filter((s) => (card.services ? card.services[s.key] !== false : true)).map((s) => (
                <button key={s.key} onClick={() => openService(s.key)} className="flex flex-col items-center gap-1.5 py-3 rounded-[14px]" style={{ background: t.bgSoft, border: `1px solid ${t.border}` }}>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <p className="text-[12.5px] font-semibold" style={{ color: t.ink }}>{VIRTUAL_CARD_SERVICES.find((s) => s.key === activeService)?.label}</p>
                <button onClick={() => setActiveService(null)} className="text-[11.5px] font-semibold" style={{ color: t.accent }}>Change</button>
              </div>

              {activeService === "lipa" && (
                <>
                  <Field t={t} label="TIPS till or phone number"><input style={inputStyle(t)} value={lipaDest} onChange={(e) => setLipaDest(e.target.value)} placeholder="e.g. 0712 345 678" /></Field>
                  <Field t={t} label="Amount"><input style={inputStyle(t)} type="number" value={lipaAmt} onChange={(e) => setLipaAmt(e.target.value)} /></Field>
                  <Field t={t} label="Category"><select style={inputStyle(t)} value={lipaCategory} onChange={(e) => setLipaCategory(e.target.value)}>{categories.filter((c) => c.id && (!card.allowedCategoryIds || card.allowedCategoryIds.includes(c.id))).map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></Field>
                  <PrimaryButton t={t} disabled={!lipaDest || !lipaAmt || !lipaCategory || busy} onClick={() => run(async () => { const r = await api.virtualCards.payLipa(card.id, lipaDest, parseFloat(lipaAmt), lipaCategory); setActiveService(null); return r; })}>{busy ? "Paying…" : "Pay"}</PrimaryButton>
                </>
              )}

              {activeService === "gepg" && (
                <>
                  <Field t={t} label="GePG control number"><input style={inputStyle(t)} value={gepgControl} onChange={(e) => { setGepgControl(e.target.value.replace(/\D/g, "")); setGepgBill(null); }} placeholder="e.g. 991234567" /></Field>
                  {!gepgBill ? (
                    <PrimaryButton t={t} tone={t.gold} disabled={gepgControl.length < 6} onClick={checkGepgBill}>Check bill</PrimaryButton>
                  ) : (
                    <>
                      <div className="p-3 rounded-[14px] mb-3" style={{ background: t.goldSoft }}><p className="text-[13px] font-bold" style={{ color: t.ink }}>{gepgBill.biller}</p><p className="text-[16px] font-bold mt-1" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(gepgBill.amount)}</p></div>
                      <Field t={t} label="Category"><select style={inputStyle(t)} value={gepgCategory} onChange={(e) => setGepgCategory(e.target.value)}>{categories.filter((c) => c.id && (!card.allowedCategoryIds || card.allowedCategoryIds.includes(c.id))).map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></Field>
                      <PrimaryButton t={t} tone={t.gold} disabled={busy} onClick={() => run(async () => { const r = await api.virtualCards.payGepg(card.id, gepgControl, gepgBill.amount, gepgBill.biller, gepgCategory); setActiveService(null); return r; })}>{busy ? "Paying…" : "Pay now"}</PrimaryButton>
                    </>
                  )}
                </>
              )}
                ...
                <>
                  {lukuToken ? (
                    <div className="text-center py-3">
                      <p className="text-[12px] mb-1" style={{ color: t.inkFaint }}>Token generated</p>
                      <p className="text-[17px] font-bold tracking-wider" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{lukuToken}</p>
                    </div>
                  ) : (
                    <>
                      <Field t={t} label="Meter number"><input style={inputStyle(t)} value={lukuMeter} onChange={(e) => setLukuMeter(e.target.value.replace(/\D/g, ""))} placeholder="e.g. 01234567891" /></Field>
                      <Field t={t} label="Amount"><input style={inputStyle(t)} type="number" value={lukuAmt} onChange={(e) => setLukuAmt(e.target.value)} /></Field>
                      <Field t={t} label="Category"><select style={inputStyle(t)} value={lukuCategory} onChange={(e) => setLukuCategory(e.target.value)}>{categories.filter((c) => c.id && (!card.allowedCategoryIds || card.allowedCategoryIds.includes(c.id))).map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></Field>
                      <PrimaryButton t={t} tone="#4A8C8C" disabled={!lukuMeter || !lukuAmt || busy} onClick={() => run(async () => { const r = await api.virtualCards.payLuku(card.id, lukuMeter, parseFloat(lukuAmt), lukuCategory); setLukuToken(r.token); return r; })}>{busy ? "Buying…" : "Buy token"}</PrimaryButton>
                    </>
                  )}
                </>
              )}
...
function VirtualCardsPage({ t, goBack, categories, walletsList, user, tr, onMainCardChanged }) {
  ...
  const load = async () => {
    setLoading(true);
    try { setCards(await api.virtualCards.list()); } catch (err) { console.error("Failed to load virtual cards", err); } finally { setLoading(false); } // eslint-disable-line no-console
  };
  useEffect(() => { load(); }, []);

  const createIndependent = async () => {
    setCreatingIndependent(true);
    try { await api.virtualCards.createIndependent(); await load(); }
    catch (err) { console.error("Failed to create card", err); } // eslint-disable-line no-console
    finally { setCreatingIndependent(false); }
  };

  const nameFor = (userId) => sharedWallet?.members?.find((m) => m.userId === userId)?.user?.firstName || "Member";

  const independent = cards.filter((c) => c.type === "independent");
  ...
  return (
    <div className="px-4 pt-2 pb-24">
      <div className="flex items-center gap-2 mb-5">
        <button onClick={goBack} className="p-1.5 -ml-1.5 rounded-full" style={{ background: t.cardSoft }}><ArrowLeft size={16} color={t.inkSoft} /></button>
        <h1 className="text-[20px] font-bold" style={{ color: t.ink, fontFamily: "'Space Grotesk', sans-serif" }}>Virtual cards</h1>
      </div>
      ...
      {loading ? (
        ...
        <>
          {independent.length > 0 && (
            <>
              <SectionLabel t={t}>My independent card{independent.length > 1 ? "s" : ""}</SectionLabel>
              {independent.map((c) => <VirtualCardTile key={c.id} t={t} card={c} onOpen={() => setDetailCard(c)} />)}
            </>
          )}
          ...
          {cards.length === 0 && <p className="text-center text-[13px] py-6" style={{ color: t.inkFaint }}>No virtual cards yet.</p>}

          <div className="space-y-2 mt-2">
            <button onClick={createIndependent} disabled={creatingIndependent} className="w-full flex items-center justify-between p-4 rounded-[18px]" style={{ background: t.cardSoft, border: `1px solid ${t.border}` }}>
              <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: t.accentSoft }}><Plus size={16} color={t.good} /></div><div className="text-left"><p className="text-[13.5px] font-semibold" style={{ color: t.ink }}>{creatingIndependent ? "Creating…" : "Create independent card"}</p><p className="text-[11.5px]" style={{ color: t.inkFaint }}>Personal — funded and controlled only by you</p></div></div>
            </button>
            {isSharedOwner && invitableMembers.length > 0 && (
              <button onClick={() => setIssueSheetOpen(true)} className="w-full flex items-center justify-between p-4 rounded-[18px]" style={{ background: t.goldSoft, border: `1px solid ${t.border}` }}>
...
function ProfilePage({ t, user, avatarUrl, setAvatarUrl, goBack, tr, onLogout, onUpdateProfileName }) {
  const fileRef = useRef(null);
  const handleFile = (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    <div style={{ background: t.bgSoft, minHeight: "100vh", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="max-w-[430px] mx-auto relative" style={{ background: t.bg, minHeight: "100vh" }}>
        ...
        <div style={{ paddingBottom: 84 }}>
          ...
          {active === "pay" && (
            <PayPage t={t} transactions={transactions} categories={categories} categoryIdByName={categoryIdByName} refreshCardAndTransactions={refreshCardAndTransactions} user={user}
          ...
          {active === "virtualcards" && <VirtualCardsPage t={t} goBack={() => setActive("wallets")} categories={categories} walletsList={walletsList} user={user} tr={tr} onMainCardChanged={refreshCardAndTransactions} />}
          {active === "profile" && <ProfilePage t={t} user={user} avatarUrl={avatarUrl} setAvatarUrl={setAvatarUrl} goBack={() => setActive(profileReturnTab)} tr={tr} onLogout={handleLogout} onUpdateProfileName={updateProfileName} />}
