...
function WalletsPage({ t, transactions, onAdd, walletsList, user, myInvites, onAcceptInvite, onDeclineInvite, onCreateSharedWallet, onRefreshWallets, setActive, categories, onAddCategory, onAddSubcategory, tr }) {
  ...
  const [selectedMember, setSelectedMember] = useState(null); // { userId, name } | null
  ...
  return (
    <div className="px-4 pt-2 pb-24 relative">
      ...
      {hasShared && activeWallet === "shared" && sharedWallet.members?.length > 0 && (
        <MemberSpendCard t={t} transactions={transactions} members={sharedWallet.members} user={user} onSelectMember={(userId, name) => setSelectedMember({ userId, name })} />
      )}
      ...
      <div className="flex items-center justify-between mb-1">
        ...
        {sorted.length > 4 && (
          <button onClick={() => setShowActivityPage(true)} className="text-[11.5px] font-semibold" style={{ color: t.accent }}>View all</button>
        )}
      </div>
      ...
      {showActivityPage && (
        <div className="fixed inset-0 z-50" style={{ background: t.bg }}>
          <WalletActivityPage
            t={t} transactions={transactions} categories={categories} wallet={activeWallet}
            walletLabel={activeWallet === "personal" ? tr("recentTransactions") : tr("sharedActivity")}
            showLogger={activeWallet === "shared"} goBack={() => setShowActivityPage(false)}
          />
        </div>
      )}
      {selectedMember && (
        <div className="fixed inset-0 z-50" style={{ background: t.bg }}>
          <WalletActivityPage
            t={t} transactions={transactions} categories={categories} wallet="shared"
            filterUserId={selectedMember.userId} filterUserName={selectedMember.name}
            showLogger={false} goBack={() => setSelectedMember(null)}
          />
        </div>
      )}
