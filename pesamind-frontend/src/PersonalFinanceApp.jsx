...
function ChargesLine({ t, view }) {
  if (!view.transfersEnabled) {
    return <p className="text-[12px]" style={{ color: t.danger }}>Transfers to this institution are currently unavailable.</p>;
  }
  const captionParts = [];
  if (view.charge > 0) captionParts.push(`Charge ${fmtTZS(view.charge)}`);
  if (view.vat > 0) captionParts.push(`${view.vatLabel} ${fmtTZS(view.vat)}`);

  return (
    <div className="mb-1">
      <div className="flex items-center justify-between">
        <span className="flex items-center gap-1.5 text-[12px]" style={{ color: t.inkFaint }}>
          Charges {(captionParts.length > 0 || view.coveredMessage) && <Info size={12} color={t.inkFaint} />}
        </span>
        <span className="text-[12px] font-semibold" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
          {view.isWaived && view.wouldBeTotalCharges > 0 ? (
            <>
              <span style={{ textDecoration: "line-through", color: t.inkFaint, marginRight: 6 }}>{fmtTZS(view.wouldBeTotalCharges)}</span>
              <span style={{ color: t.good }}>FREE</span>
            </>
          ) : (
            <span style={{ color: t.ink }}>{view.totalCharges > 0 ? fmtTZS(view.totalCharges) : "FREE"}</span>
          )}
        </span>
      </div>
      {view.coveredMessage ? (
    ...
    <BottomSheet t={t} open={open} onClose={onClose} title="Lipa">
      ...
      {phase === "confirm" && resolved && (
        <div>
              ...
              <div className="p-3.5 rounded-[14px] mb-4" style={{ background: t.bgSoft, border: `1px solid ${t.border}` }}>
                {!view.transfersEnabled ? (
                  ...
                  <>
                    ...
                    <ChargesLine t={t} view={view} />
                    <div className="flex justify-between pt-1.5 mt-1" style={{ borderTop: `1px solid ${t.border}` }}><span className="text-[12.5px] font-semibold" style={{ color: t.ink }}>Total amount</span><span className="text-[13px] font-bold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(amt + view.totalCharges)}</span></div>
                  </>
                )}
              </div>
            );
          ...
          {error && <p className="text-[12.5px] mb-3" style={{ color: t.danger }}>{error}</p>}
          <PrimaryButton t={t} onClick={goToAuth} disabled={resolved.fee && !getCustomerChargesView(getFeeBreakdown(resolved.fee, amt)).transfersEnabled}>Continue</PrimaryButton>
        </div>
      )}
