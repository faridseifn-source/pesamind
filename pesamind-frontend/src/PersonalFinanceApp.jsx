...
function QrPaymentSheet({ t, open, onClose, cardType, cardId, categories, kycVerified, setKycVerified, onSuccess, tr, language }) {
  ...
  // QR scanning takes a single photo via the native camera (the same
  // file-input mechanism already confirmed working for receipt scanning)
  // rather than a live video stream. getUserMedia-based live scanning
  // proved unreliable inside Capacitor's iOS WebView even after the
  // documented fixes (secure-context scheme config) - navigator.mediaDevices
  // stayed undefined regardless. This trades live auto-detection for a
  // "snap a photo, then decode it" flow using a mechanism already proven
  // to work reliably.
  const handleFileCapture = (e) => {
    const file = e.target.files?.[0];
    e.target.value = ""; // allow capturing again even if the same photo would otherwise be selected
    if (!file) return;
    ...
    img.onload = () => {
      ...
      if (code?.data) {
        ...
        setScanError("No QR code found in that photo. Try again, making sure the code fills the frame and is in focus.");
      }
    };
    img.onerror = () => {
      URL.revokeObjectURL(objectUrl);
      setScanning(false);
      setScanError("Couldn't read that photo. Please try again.");
    };
    img.src = objectUrl;
  };
          ...
          {entryMode === "camera" && (
            <div className="mb-4">
              ...
              >
                ...
                <p className="text-[13.5px] font-semibold" style={{ color: t.ink }}>{scanning ? "Reading photo…" : "Tap to scan a QR code"}</p>
                <p className="text-[11.5px] max-w-[240px] text-center" style={{ color: t.inkFaint }}>Opens your camera — take a clear photo with the QR code filling the frame.</p>
              </button>
