...
// — used to show a live "Total charges" estimate as the customer types an
// amount, without a network round-trip per keystroke. The backend
// recomputes and enforces this for real at payment time regardless.
      ...
      <PaySuccess
        ...
        extra={hasFee && (
          <div className="flex justify-between py-1.5" style={{ borderTop: `1px solid ${t.border}` }}>
            <span className="text-[11.5px]" style={{ color: t.inkFaint }}>Total charges</span>
            <span className="text-[11.5px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(payment.feeAmount)}</span>
          </div>
        )}
      />
    );
    ...
    <BottomSheet t={t} open={open} onClose={onClose} title="Lipa">
      ...
      {phase === "confirm" && resolved && (
        <div>
            ...
            <div className="p-3.5 rounded-[14px] mb-4" style={{ background: t.bgSoft, border: `1px solid ${t.border}` }}>
              {!resolved.fee.transfersEnabled ? (
                ...
                <>
                  ...
                  <div className="flex justify-between mb-1"><span className="text-[12px]" style={{ color: t.inkFaint }}>Total charges</span><span className="text-[12px] font-semibold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{computeFeeEstimate(resolved.fee, amt) > 0 ? fmtTZS(computeFeeEstimate(resolved.fee, amt)) : "FREE"}</span></div>
                  <div className="flex justify-between pt-1.5" style={{ borderTop: `1px solid ${t.border}` }}><span className="text-[12.5px] font-semibold" style={{ color: t.ink }}>Total to pay</span><span className="text-[13px] font-bold" style={{ color: t.ink, fontFamily: "'IBM Plex Mono', monospace" }}>{fmtTZS(amt + computeFeeEstimate(resolved.fee, amt))}</span></div>
                </>
              )}
            </div>
          )}
          ...
          {error && <p className="text-[12.5px] mb-3" style={{ color: t.danger }}>{error}</p>}
          <PrimaryButton t={t} onClick={goToAuth} disabled={!resolved.isOnUs && resolved.fee && !resolved.fee.transfersEnabled}>Continue</PrimaryButton>
        </div>
      )}
