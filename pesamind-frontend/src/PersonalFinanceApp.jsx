...
function WalletCardStack({ t, cards, selectedId, onSelect }) {
  ...
  const select = (id) => { onSelect(id); setExpanded(false); };

  const handleDragEnd = (event, info) => {
    if (info.offset.y < -28 || info.velocity.y < -250) setExpanded(true);
    else if (info.offset.y > 28 || info.velocity.y > 250) setExpanded(false);
    fmAnimate(handleY, 0, { type: "spring", stiffness: 420, damping: 32 });
  };
    ...
    <div className="mb-4">
      ...
      {ordered.length > 1 && (
        <motion.div
          ...
          className="flex justify-center pt-2.5 cursor-grab active:cursor-grabbing touch-none"
        >
          <div className="w-9 h-1.5 rounded-full" style={{ background: t.border }} />
        </motion.div>
      )}
    </div>
  );
