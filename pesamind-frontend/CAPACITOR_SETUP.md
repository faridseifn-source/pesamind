...
## Biometric login / passkeys (Android)

This replaces the previous "Face ID" attempt, which failed on iOS
because Apple's Associated Domains capability requires a paid Apple
Developer account. Android's equivalent (Digital Asset Links) doesn't
have that paid-account requirement - but it does need a real, public
domain, and the app's existing WebAuthn code (which talks to a plain
browser API that genuinely isn't supported inside an embedded Android
WebView, confirmed directly from Google's own passkeys documentation)
needed a plugin that bridges to Android's real native Credential
Manager instead. `@capgo/capacitor-passkey` does this as a "shim" -
it intercepts the app's existing `navigator.credentials.create()`/`get()`
calls (used internally by `@simplewebauthn/browser`, already in this
app) and forwards them natively, so the actual login/registration
screens didn't need rewriting.

**Important**: this is tied to `pesamind.vercel.app` - the domain
already set as `WEBAUTHN_RP_ID` on Render, matching the website's
existing biometric login. Any future move to a custom domain will
require regenerating `assetlinks.json`, updating `capacitor.config.json`,
and updating `WEBAUTHN_RP_ID` together - all existing web and native
passkeys would need to be re-registered after that kind of change,
since a passkey is permanently tied to the relying-party domain it was
created under.
...
### Step 2: Extract your Android app's signing certificate fingerprint

This identifies your specific build to Android's Digital Asset Links
system. For testing with a debug build:

```
keytool -list -v -keystore ~/.android/debug.keystore -alias androiddebugkey -storepass android -keypass android
```

Look for the line starting with `SHA256:` in the output - a string of
colon-separated hex pairs, e.g.
`AA:BB:CC:DD:EE:FF:00:11:22:33:44:55:66:77:88:99:AA:BB:CC:DD:EE:FF:00:11:22:33:44:55:66:77:88:99`.
Copy the whole thing.

**Important caveat**: this debug keystore's fingerprint is specific to
this development machine. Once you eventually build a signed release
for the Play Store, that release key will have a *different*
fingerprint - `assetlinks.json` will need updating again at that point
to include both (debug and release can both be listed in the same
`sha256_cert_fingerprints` array, so this isn't a replace, it's an
addition).

### Step 3: Update `assetlinks.json` with the real fingerprint

Open `pesamind-frontend/public/.well-known/assetlinks.json` and
replace the placeholder string with the real fingerprint from Step 2:
...
### Step 4: Deploy the frontend and verify the file is actually served

Push this change and let Vercel redeploy. Then check directly in a
browser:

```
https://pesamind.vercel.app/.well-known/assetlinks.json
```

You should see the JSON content, not a 404. **Note**: this specific
path is known to not work correctly on Vite's local dev server (a
confirmed Vite limitation with dot-folder paths) - always verify
against the real deployed Vercel URL, not `localhost`, or this step
will look broken when it actually isn't.

### Step 5: Build and test on a real Android phone
...
Run on your real phone (not the emulator - biometric enrollment needs
real fingerprint/face hardware). In the app:

1. Log in normally with your password first
2. Go to Profile → Biometric login (or wherever the "Set up Face ID /
   biometric login" option lives) and enroll this device
3. Log out, then try logging back in using the biometric option

### Step 6: If registration/login fails with a verification error

Keep Android Studio's Logcat open (filter for `Capacitor`) while
testing, and check the backend's Render logs at the same time. The
most likely failure at this stage is the backend rejecting Android's
distinct native origin string during verification - this is the one
detail that's genuinely hard to know in advance, since the exact
format can vary slightly by OS/plugin version.

If you see a verification failure, look in the backend logs or a
Logcat network log for the actual origin string Android sent (it will
look something like `android:apk-key-hash:<some-hash>`). Then, on
Render, set:

- Key: `WEBAUTHN_EXTRA_ORIGINS`
- Value: the exact origin string you found

Save (Render redeploys automatically), then try again. No code change
needed for this - it was deliberately built as a configurable env var
